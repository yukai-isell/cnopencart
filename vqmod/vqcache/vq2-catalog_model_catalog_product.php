...
class ModelCatalogProduct extends Model {
		...
		public function getPromotionProducts($data = array()) {
			...
			if (true) {
				...
				$sql .= " AND p.product_id IN (SELECT product_id FROM  " . DB_PREFIX . "product_special WHERE promotion=1 AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW()))) ";
	...
	public function getProducts($data = array()) {
		...
		return $product_data;
			...
			public function getProductSpecialPromotion($product_id){
				if ($this->customer->isLogged()) {
					$customer_group_id = $this->customer->getCustomerGroupId();
				} else {
					$customer_group_id = $this->config->get('config_customer_group_id');
				...
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$customer_group_id . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND promotion=1  ORDER BY priority ASC, price ASC");
